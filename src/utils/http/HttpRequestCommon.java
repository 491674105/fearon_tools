...
public class HttpRequestCommon {
    private static final Logger LOGGER = LoggerFactory.getLogger(HttpRequestCommon.class);
    ...
    public static String doGet(String uri, String params) {
        ...
        try {
            ...
            if (code == 404) {
                LOGGER.error("认证无效，找不到此次认证的会话信息！");
            }
            if (code == 500) {
                LOGGER.error("认证服务器发生内部错误！");
            }
            if (code != 200) {
                LOGGER.error("发生其它错误，认证服务器返回 " + code);
            }
            ...
            LOGGER.error("没有指定通信协议！");
        } catch (IOException e) {
            LOGGER.error("数据流开启异常！");
        } catch (NullPointerException e) {
            LOGGER.error("未读取到有效数据流！");
        } finally {
            if(reader != null) {
                try {
                    reader.close();
